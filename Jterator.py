...
def JModules_create(function_names, input_args):
    ...
    for module in function_names:
        # to do:
        #   - variable input is a sting with the name of the module
        #   - parameters specified in json file
        #   - create log files
        if function_names[module][-2:] == '.m':
            commands[module] = 'matlab -nodisplay -r "%s(%s); exit"' %(function_names[module][:-3], module)
        elif function_names[module][-2:] == '.R':
            commands[module] =  'R CMD BATCH -%s %s' %(module,function_names[module]) # use commandArgs() in R
        elif function_names[module][-3:] == '.py':
            commands[module] = 'python -c"%(function_name)s(%(function_arg)s)"' % {
                'function_name': function_names[module], 'function_arg': module}
